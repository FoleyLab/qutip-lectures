...
class HolsteinTavisCummings:
    ...
    def build_polaron_basis(self):
        ...
        # H_pt = self.H_qubit + self.H_vibrational + self.H_qubit_vibrational_coupling 
        # H_pt = self.H_qubit + self.H_vibrational + self.H_qubit_vibrational_coupling + self.H_cav
        # vals, vecs = H_pt.eigenstates()

        if self.include_cavity:
            H_pt = self.H_qubit + self.H_vibrational + self.H_qubit_vibrational_coupling + self.H_cav
            vals, vecs = H_pt.eigenstates()
            H_pt += self.H_qubit_cavity_coupling
        
        else:
            H_pt = self.H_qubit + self.H_vibrational + self.H_qubit_vibrational_coupling
            vals, vecs = H_pt.eigenstates()
            #add cavity Hamiltonian and qubit-cavity coupling to Hamiltonian
            H_pt += self.H_qubit_cavity_coupling + self.H_cav

        #Build matrix from qubit eigenvectors
        vecs_new = [vec.full() for vec in vecs]
        self.H_vecs = Qobj(np.hstack(vecs_new),dims=H_pt.dims)

        #Perform polaron transformation
        self.H_p = self.H_vecs.dag() * H_pt * self.H_vecs

    
    def build_polariton_basis(self):
        """Method to build the polariton basis for the system."""
        H_pt = self.H_qubit = self.H_cav + self.H_qubit_cavity_coupling
        vals, vecs = H_pt.eigenstates()

        H_pt += self.H_qubit_vibrational_coupling + self.H_vibrational
        # Build matrix from qubit eigenvectors
        vecs_new = [vec.full() for vec in vecs]
        self.H_vecs_polariton = Qobj(np.hstack(vecs_new), dims=H_pt.dims)
        # Perform polariton transformation
        self.H_polariton = self.H_vecs.dag() * H_pt * self.H_vecs
        
    @staticmethod
    def wt(t_, T_1, T_g_1):
        if t_ > T_1 and t_ <= T_1 + T_g_1:
            return 1
        return 0
    ...
    def build_polaron_basis_t(self, t, t1, t_gate_1, t2, t_gate_2, return_vectors=False):
        ...
        # time dependent qubit-cavity coupling
        H_q1c = self.wt(t, t1, t_gate_1) * self.H_q1_cav
        H_q2c = self.wt(t, t2, t_gate_2) * self.H_q2_cav

        #build polaron transformation Hamiltonian

        H_change = self.H_qubit + self.H_vibrational + self.H_qubit_vibrational_coupling + self.H_cav + H_q1c + H_q2c

        #Perform polaron transformation
        #self.H_polaron = self.H_vectors.dag() * self.H_change * self.H_vectors
        self.H_polaron = self.H_vecs.dag() * H_change * self.H_vecs
    ...
    def build_time_dependent_bare_Hamiltonian(self, t, t1, t_gate_1, t2, t_gate_2):
        ...
        self.H_bare = self.H_qubit + self.H_vibrational + self.H_qubit_vibrational_coupling + self.H_cav + H_q1c + H_q2c

        return self.H_bare
    
    def build_time_dependent_polariton_Hamiltonian(self, t, t1, t_gate_1, t2, t_gate_2):
        """Method to build the time-dependent polariton Hamiltonian for the system at a given time t.
        ...
        Args:
            ...
            t_gate_2 (float): Duration of the second qubit frequency change.
        Returns:
            H_polariton (Qobj): The polariton Hamiltonian at time t.
        """
        H_q1c = self.wt(t, t1, t_gate_1) * self.H_q1_cav
        H_q2c = self.wt(t, t2, t_gate_2) * self.H_q2_cav

        H_change = self.H_qubit + self.H_vibrational + self.H_qubit_vibrational_coupling + H_q1c + H_q2c + self.H_cav

        self.H_polariton = self.H_vecs_polariton.dag() * H_change * self.H_vecs_polariton

        return self.H_polariton
