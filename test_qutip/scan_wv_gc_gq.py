...
gc_values = np.linspace(0.001, 0.1, 10)   # qubit–cavity coupling
...
# -----------------------------------------------------------
with open("scan_results_wv_gc_gv.csv", "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["wv", "gc", "gv", "Fidelity", "Concurrence"])

    for wv in wv_values:
        # bare Hamiltonians (update vibrational part each time)
        ...
        for gc in gc_values:
            for gv in gv_values:
                ...
                writer.writerow([wv, gc, gv, fide, conc])
                print(f"wv={wv:.3f}, gc={gc:.3f}, gv={gv:.4f}, F={fide:.6f}, C={conc:.6f}")
